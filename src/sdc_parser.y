...
cmd_create_clock: CMD_CREATE_CLOCK                          { $$ = alloc_sdc_create_clock(); }
    ...
    | cmd_create_clock string                               { $$ = sdc_create_clock_add_targets($1, make_sdc_string_group(t_sdc_string_group_type::SDC_STRING, $2)); free($2); }
    ;

cmd_set_input_delay: CMD_SET_INPUT_DELAY        { $$ = alloc_sdc_set_io_delay(t_sdc_io_delay_type::SDC_INPUT_DELAY); }
...
cmd_set_output_delay: CMD_SET_OUTPUT_DELAY       { $$ = alloc_sdc_set_io_delay(t_sdc_io_delay_type::SDC_OUTPUT_DELAY); }
...
cmd_set_clock_groups: CMD_SET_CLOCK_GROUPS                  { $$ = alloc_sdc_set_clock_groups(); }
    | cmd_set_clock_groups ARG_EXCLUSIVE                    { $$ = sdc_set_clock_groups_set_type($1, t_sdc_clock_groups_type::SDC_CG_EXCLUSIVE); }
    ...
    | cmd_set_clock_groups ARG_GROUP     string             { $$ = sdc_set_clock_groups_add_group($1, make_sdc_string_group(t_sdc_string_group_type::SDC_STRING, $3)); free($3); }
    ;

cmd_set_false_path: CMD_SET_FALSE_PATH                      { $$ = alloc_sdc_set_false_path(); }
    | cmd_set_false_path ARG_FROM '[' cmd_get_clocks ']'    { $$ = sdc_set_false_path_add_to_from_group($1, $4, t_sdc_to_from_dir::SDC_FROM); free_sdc_string_group($4); }
    | cmd_set_false_path ARG_TO   '[' cmd_get_clocks ']'    { $$ = sdc_set_false_path_add_to_from_group($1, $4, t_sdc_to_from_dir::SDC_TO  ); free_sdc_string_group($4); }
    | cmd_set_false_path ARG_FROM '{' stringGroup '}'       { $$ = sdc_set_false_path_add_to_from_group($1, $4, t_sdc_to_from_dir::SDC_FROM); free_sdc_string_group($4); }
    | cmd_set_false_path ARG_TO   '{' stringGroup '}'       { $$ = sdc_set_false_path_add_to_from_group($1, $4, t_sdc_to_from_dir::SDC_TO  ); free_sdc_string_group($4); }
    | cmd_set_false_path ARG_FROM     string                { $$ = sdc_set_false_path_add_to_from_group($1, make_sdc_string_group(t_sdc_string_group_type::SDC_STRING, $3), t_sdc_to_from_dir::SDC_FROM); free($3); }
    | cmd_set_false_path ARG_TO       string                { $$ = sdc_set_false_path_add_to_from_group($1, make_sdc_string_group(t_sdc_string_group_type::SDC_STRING, $3), t_sdc_to_from_dir::SDC_TO  ); free($3); }
    ;
...
cmd_set_max_delay: CMD_SET_MAX_DELAY                        { $$ = alloc_sdc_set_max_delay(); }
    ...
    | cmd_set_max_delay ARG_FROM '[' cmd_get_clocks ']'     { $$ = sdc_set_max_delay_add_to_from_group($1, $4, t_sdc_to_from_dir::SDC_FROM); free_sdc_string_group($4); }
    | cmd_set_max_delay ARG_TO   '[' cmd_get_clocks ']'     { $$ = sdc_set_max_delay_add_to_from_group($1, $4, t_sdc_to_from_dir::SDC_TO  ); free_sdc_string_group($4); }
    | cmd_set_max_delay ARG_FROM '{' stringGroup '}'        { $$ = sdc_set_max_delay_add_to_from_group($1, $4, t_sdc_to_from_dir::SDC_FROM); free_sdc_string_group($4); }
    | cmd_set_max_delay ARG_TO   '{' stringGroup '}'        { $$ = sdc_set_max_delay_add_to_from_group($1, $4, t_sdc_to_from_dir::SDC_TO  ); free_sdc_string_group($4); }
    | cmd_set_max_delay ARG_FROM     string                 { $$ = sdc_set_max_delay_add_to_from_group($1, make_sdc_string_group(t_sdc_string_group_type::SDC_STRING, $3), t_sdc_to_from_dir::SDC_FROM); free($3); }
    | cmd_set_max_delay ARG_TO       string                 { $$ = sdc_set_max_delay_add_to_from_group($1, make_sdc_string_group(t_sdc_string_group_type::SDC_STRING, $3), t_sdc_to_from_dir::SDC_TO  ); free($3); }
    ;
...
cmd_set_multicycle_path: CMD_SET_MULTICYCLE_PATH                { $$ = alloc_sdc_set_multicycle_path(); }
    ...
    | cmd_set_multicycle_path ARG_SETUP                         { $$ = sdc_set_multicycle_path_set_type($1, t_sdc_mcp_type::SDC_MCP_SETUP); }
    | cmd_set_multicycle_path ARG_FROM '[' cmd_get_clocks ']'   { $$ = sdc_set_multicycle_path_add_to_from_group($1, $4, t_sdc_to_from_dir::SDC_FROM); free_sdc_string_group($4); }
    | cmd_set_multicycle_path ARG_TO   '[' cmd_get_clocks ']'   { $$ = sdc_set_multicycle_path_add_to_from_group($1, $4, t_sdc_to_from_dir::SDC_TO  ); free_sdc_string_group($4); }
    | cmd_set_multicycle_path ARG_FROM '{' stringGroup '}'      { $$ = sdc_set_multicycle_path_add_to_from_group($1, $4, t_sdc_to_from_dir::SDC_FROM); free_sdc_string_group($4); }
    | cmd_set_multicycle_path ARG_TO   '{' stringGroup '}'      { $$ = sdc_set_multicycle_path_add_to_from_group($1, $4, t_sdc_to_from_dir::SDC_TO  ); free_sdc_string_group($4); }
    | cmd_set_multicycle_path ARG_FROM     string               { $$ = sdc_set_multicycle_path_add_to_from_group($1, make_sdc_string_group(t_sdc_string_group_type::SDC_STRING, $3), t_sdc_to_from_dir::SDC_FROM); free($3); }
    | cmd_set_multicycle_path ARG_TO       string               { $$ = sdc_set_multicycle_path_add_to_from_group($1, make_sdc_string_group(t_sdc_string_group_type::SDC_STRING, $3), t_sdc_to_from_dir::SDC_TO  ); free($3); }
    ;

cmd_get_ports: CMD_GET_PORTS            { $$ = alloc_sdc_string_group(t_sdc_string_group_type::SDC_PORT); }
...
cmd_get_clocks: CMD_GET_CLOCKS              { $$ = alloc_sdc_string_group(t_sdc_string_group_type::SDC_CLOCK); }
...
stringGroup: /*empty*/   { $$ = alloc_sdc_string_group(t_sdc_string_group_type::SDC_STRING); }
