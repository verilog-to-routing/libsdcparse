...
# CHECK-NEXT: [[FFAQ0_pin_ptr:[0-9]+]]
puts [libsdcparse_create_pin "FFA.Q\[0\]" -direction OUTPUT]
# CHECK-NEXT: [[FFDin0_pin_ptr:[0-9]+]]
...
# CHECK: set_clock_latency -source {{1.0*}} {9}
set_clock_latency -source 1.0 [get_clocks {clk}]

# CHECK: set_clock_groups -asynchronous -group {11} -group {10}
set_clock_groups -asynchronous -group input_clk -group clk2

# CHECK: set_false_path -from {9} -to {12}
